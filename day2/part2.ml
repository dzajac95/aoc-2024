...
let rec check_report_filt report idx =
    if idx >= List.length report then
        false
    else if check_report (List.filteri (fun i _ -> i != idx) report) then
        true
    else
...
let () =
    ...
    let report_results = List.map (fun report -> check_report report || check_report_filt report 0) reports in
